...
def save_submission(conn, commitfest_id, submission_id):
  # add all changes
  cursor = conn.cursor()
...
def process_submission(conn, commitfest_id, submission_id):
  ...
  if rcode != 0:
    ...
    # we committed the patch now store the submission in the database
    save_submission(conn, commitfest_id, submission_id)
